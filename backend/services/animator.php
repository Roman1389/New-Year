...
try {
   ...
   elseif ($action === 'submit_order') {
      if (!isset($_POST['user_id'], $_POST['phone'], $_POST['date'], $_POST['time'], $_POST['peopleCount'], $_POST['animatorCharacter'])) {
      ...
      $animator_character = $_POST['animatorCharacter']; // Получаем выбранного аниматора
      ...
      // Преобразуем выбранного аниматора в более читабельный формат
      $animator_character_text = '';
      if ($animator_character === 'dedMoroz') {
         $animator_character_text = 'Дед Мороз';
      } elseif ($animator_character === 'snegurochka') {
         $animator_character_text = 'Снегурочка';
      } elseif ($animator_character === 'both') {
         $animator_character_text = 'Дед Мороз и Снегурочка';
      } else {
         $animator_character_text = 'Не выбран'; // Если не выбран
      }

      // Запись заказа в базу данных (аниматор не сохраняем в базе)
      $stmt = $conn->prepare("INSERT INTO orders (service_id, user_id, order_date, order_time, phone, people_count, total_price)
                                VALUES (?, ?, ?, ?, ?, ?, ?)");
      ...
      if ($stmt->execute()) {
         ...
         $message .= "Аниматоры: $animator_character_text\n"; // Выводим правильно отформатированный аниматор
